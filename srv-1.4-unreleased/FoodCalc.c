/*
  ...
  v. 1.4  1999xxxx  Jesper Lauritsen
                    Now reads number at the form 1.2e34
					More general transpose: command.
...
typedef enum {strArg, chArg, listArg, numArg, setArg, whereArg, transposeArg} ArgType;
typedef struct {
...
ArgType transposeArgs[] = {transposeArg/*field [numspec] outlist*/};
CmdDef transposeDef = {"transpose",optional,multiple,1,1,&transposeCmd,transposeArgs};
...
int getNum() { /* call skipSpace before getNum! */
	...
	if (ch == 'e' || ch == 'E') {
		...
		num *= (Num)pow(10.0,(double)(e*esign));
	}
	return(1);
}
...
/****************************************************************************/
/* functions for reading lists with the following EBNF LL(1) syntax:
	OutList	::= OutSpec {"," OutSpec}
	OutSpec	::= Field ["=" Exp]
   We use recursive descent to read the syntax. */

typedef struct OutSpec_ {
	char* name;
	Exp* exp;
	struct OutSpec_* next;
} OutSpec;
Chain(OutSpec,OutList);

forward void pOutSpec(OutList* ol);

OutList* pOutList() { /* OutSpec {"," OutSpec} */
	OutList* ol = allocStruct(OutList);
	nolink(*ol);
	pOutSpec(ol);
	while (sym == commaSym) {
		getSym();
...
void pOutSpec(OutList* ol) { /* Field ["=" Exp] */
	OutSpec* os = allocStruct(OutSpec);
	if (sym == fieldSym) {
		os->name = saveStr();
		getSym();
	} else {
...
/****************************************************************************/
/* functions for reading spec with the following EBNF LL(1) syntax:
	TransSpec  ::= Num
	           |   "(" TransSpec1 {"," TransSpec1} ")"
	TransSpec1 ::= Num ["=" String|Num]
	           |   TransVal ["=" String|Num]
			   |   Num "-" Num
...
typedef struct TransVal1_ {
	int num1;
	int num2;
	struct TransVal1_* next;
} TransVal1;
Chain(TransVal1,TransVal);

typedef enum {transValSimple, transValList} TransValType;
typedef enum {transNone,transString,transNum} TransNameType;
typedef struct TransSpec1_ {
	TransValType valType;
	union {
		int num;
		TransVal* transVal;
	} u;
	TransNameType nameType;
	union {
		int num;
		char* name;
	} v;
	struct TransSpec1_* next;
} TransSpec1;
Chain(TransSpec1,TransSpec);

forward void pTransSpec1(TransSpec* ts);
forward TransVal* pTransVal();

TransSpec* pTransSpec() { /* Num | "(" TransSpec1 {"," TransSpec1} ")" */
	TransSpec* ts = allocStruct(TransSpec);
	nolink(*ts);
	if (sym == numSym) {
		int max = (int)num;
		int n = 1;
		while (n <= max) {
			TransSpec1* ts1 = allocStruct(TransSpec1);
            ts1->valType = transValSimple;
			ts1->u.num = n;
			ts1->nameType = transNone;
			link(*ts,ts1);
			n++;
		}
		getSym();
		...
		if (sym != rparSym) errorSym("')'"); else getSym();
	} else {
		errorSym("number or '('");
	}
	endlink(*ts);
...
void pTransSpec1(TransSpec* ts) { /* Num ["=" String|Num] | TransVal ["=" String|Num] | Num "-" Num */
	TransSpec1* ts1 = allocStruct(TransSpec1);
	if (sym == numSym) {
		ts1->valType = transValSimple;
		ts1->u.num = (int)num;
		...
		if (sym == subSym) {
			...
			getSym();
			if (sym == numSym) {
				ts1->nameType = transNum;
				ts1->v.num = (int)num;
			} else if (sym == fieldSym) {
				ts1->nameType = transString;
				ts1->v.name = saveStr();
			} else {
				errorSym("number or string");
			}
			getSym();
		} else {
			ts1->nameType = transNone;
		...
		ts1->valType = transValList;
		ts1->u.transVal = pTransVal();
		if (sym == eqSym) {
			getSym();
			if (sym == numSym) {
				ts1->nameType = transNum;
...
TransVal* pTransVal() { /* "(" TransVal {"," TransVal} ")" */
	TransVal* tv = allocStruct(TransVal);
	nolink(*tv);
	if (sym != lparSym) errorSym("'(' or number"); else getSym();
	while (1) {
		TransVal1* tv1 = allocStruct(TransVal1);
		if (sym != numSym) errorSym("number");
		tv1->num1 = (int)num;
		getSym();
		if (sym == subSym) {
			getSym();
...
void logTransSpec(TransSpec* ts) {
	TransSpec1* ts1 = ts->first;
	logmsg(" ( ");
	while (ts1) {
		if (ts1->valType == transValSimple) {
			logmsg("%d",ts1->u.num);
		} else {
			TransVal1* tv1 = ts1->u.transVal->first;
			logmsg("(");
			while (tv1) {
				logmsg("%d",tv1->num1);
				if (tv1->num1 != tv1->num2) logmsg("-%d",tv1->num2);
				tv1 = tv1->next;
				if (tv1) logmsg(",");
			}
			logmsg(")");
		}
		if (ts1->nameType == transNum) logmsg(" = %d",ts1->v.num);
		else if (ts1->nameType == transString) logmsg(" = %s",ts1->v.name);
		ts1 = ts1->next;
		if (ts1) logmsg(" , ");
	}
	logmsg(" ) ");
	...
	while (1) {
		...
		while (!eol()) {
			...
			case transposeArg: {
				...
				args[argno++] = saveStr();
				getSym();
				ts =  ((sym == numSym || sym == lparSym)? pTransSpec() : NULL);
				ol = pOutList();
				if (sym != eolSym) {errorSym("end of line"); goto cmderr;}
				if (!symError && verbosity >= 80) {
					logmsg("***transpose: %s ",args[argno-1]);
					if (ts) logTransSpec(ts);
					logOutList(ol);
					logmsg("\n");
				}
				args[argno++] = (char*)ts;
...
typedef struct Field_ {
	...
	char* outname;  /* name of the field when output */
...
void main(int argc, char** argv) {
	...
	if (*argv) {
		...
		mainCommandsName = "\\jesper\\fooddansub\\eks2.fc";
